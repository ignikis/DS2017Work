...
class Word{
	...
	/**
	 * Returns true if this word starts with the phonemes given in the parameter r.
	 */
	boolean startsWith(String r){
		if(phonemes  == null)
			return false;
		String[] phones = r.split(" ");
		int Lphones   = phones.length;
		...
		if(Lphones > Lphonemes)
			return false;
		for(int i = 0; i < Lphones; i++){
			if(!phonemes[i].equals(phones[i]))
				return false;
		}

		// If we make it here, there were no alliteration conflicts
		return true;
	}
